...
namespace NHibernate.Test.NHSpecificTest.NH3954
{
	...
	public class ProxyCacheFixture
	{
		...
		public void SetUp()
		{
			...
			_cache.StoreProxyType(typeof(Entity4FakeProxy).GetTypeInfo(), typeof(Entity4), typeof(IFieldInterceptorAccessor));
			_cache.StoreProxyType(typeof(Entity5FakeProxy).GetTypeInfo(), typeof(Entity5), typeof(INHibernateProxy), typeof(IFieldInterceptorAccessor));

			// Artificially inject other entries with same hashcodes
			_hashCode1 = new ProxyCacheEntry(typeof(Entity1), null).GetHashCode();
			Inject(new ProxyCacheEntry(typeof(Entity1), new[] { typeof(INHibernateProxy) }), _hashCode1, typeof(Entity1FakeProxy2).GetTypeInfo());
			Inject(new ProxyCacheEntry(typeof(Entity3), null), _hashCode1, typeof(Entity3FakeProxy2).GetTypeInfo());

			_hashCode2 = new ProxyCacheEntry(typeof(Entity2), new[] { typeof(INHibernateProxy) }).GetHashCode();
			Inject(new ProxyCacheEntry(typeof(Entity2), null), _hashCode2, typeof(Entity2FakeProxy2).GetTypeInfo());
			Inject(new ProxyCacheEntry(typeof(Entity4), new[] { typeof(IFieldInterceptorAccessor) }), _hashCode2, typeof(Entity4FakeProxy2).GetTypeInfo());
			Inject(new ProxyCacheEntry(typeof(Entity5), new[] { typeof(INHibernateProxy), typeof(IFieldInterceptorAccessor) }), _hashCode2, typeof(Entity5FakeProxy2).GetTypeInfo());
		}

		private void Inject(ProxyCacheEntry entryToTweak, int hashcode, TypeInfo result)
		{
		...
		{
			var result = _cache.GetProxyType(typeof(Entity4), typeof(IFieldInterceptorAccessor));
		...
		{
			var entry = new ProxyCacheEntry(typeof(Entity4), new[] { typeof(IFieldInterceptorAccessor) });
			TweakEntry(entry, _hashCode2);
			var result = _internalCache[entry];
		...
		{
			var result = _cache.GetProxyType(typeof(Entity5), typeof(IFieldInterceptorAccessor), typeof(INHibernateProxy));
		...
		public void ProxyCacheEntity5FakeProxy2()
		{
			...
			var entry = new ProxyCacheEntry(typeof(Entity5), new[] { typeof(IFieldInterceptorAccessor), typeof(INHibernateProxy) });
			TweakEntry(entry, _hashCode2);
			var result = _internalCache[entry];
		...
		public void ProxyCacheNone()
		{
			...
			TypeInfo result;
			Assert.IsFalse(_cache.TryGetProxyType(typeof(Entity2), new[] { typeof(IFieldInterceptorAccessor) }, out result));
		}
	}
}
